...
def get_filters():
    ...
    """(str) month - name of the month to filter by, or "all" to apply no month filter"""    
    month=input("Would you like to filter time by month or all months? ").capitalize()
    if month !="All":
        while month not  in ['Jan','Feb','Mar','Apr','May','Jun']:
            month=input('please select from the following[jan,feb,mar,apr,may,jun]').capitalize()
            if month=="All":
                break
    """"(str) day - name of the day of week to filter by, or "all" to apply no day filter"""
    day=input("Would you like to filter by day or all days ? ").capitalize()
    if day !="All":
        while day not in ['Saturday',"Sunday",'Monday','Tuesday','Wednesday','Thursday','Friday']:
            day=input('Please select from the following options [Saturday,Sunday,Monday,Tuesday,Wednesday,Thursday,Friday] ').capitalize()
            if day=="All":
                break
    print('-'*40)
